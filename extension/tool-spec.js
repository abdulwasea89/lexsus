...
(() => {
  ...
  const TOOLS = [
    ...
    },
    {
      ...
      args: [
        { name: "pattern", hint: "regex", required: true },
        { name: "path", hint: "path?", required: false },
        { name: "case_sensitive", hint: "case_sensitive?", required: false, type: "bool" },
      ],
      ...
      lineRe: /grep\s*[(:]\s*["']([^"']+)["']/i,
      lineArgs: ["pattern"],
      stage: { verb: "Searching", arg: "pattern", fallback: "the files" },
    },
    {
      name: "glob",
      variant: "Glob",
      ...
      args: [
        { name: "pattern", hint: "glob", required: true },
        { name: "path", hint: "path?", required: false },
      ],
      ...
      lineRe: /glob\s*[(:]\s*["']([^"']+)["']/i,
      lineArgs: ["pattern"],
      stage: { verb: "Matching", arg: "pattern", fallback: "files" },
    },
    {
      name: "git_status",
      variant: "GitStatus",
    ...
    },
    {
      ...
      args: [{ name: "path", hint: "path?", required: false }],
      summary: "Show the working-tree diff against the last commit",
      group: "Git",
      approval: "auto",
      autoInsert: true,
      timeoutMs: 15000,
      lineRe: /git_diff\s*\(\s*\)/i,
      lineArgs: [],
      stage: { verb: "Fetching", noun: "git diff" },
    },
    {
      name: "git_log",
      variant: "GitLog",
      aliases: ["log", "git_history"],
      args: [{ name: "limit", hint: "limit?", required: false, type: "int" }],
      summary: "Show recent commit history",
      group: "Git",
      approval: "auto",
      autoInsert: true,
      timeoutMs: 10000,
      lineRe: /git_log\s*\(\s*\)/i,
      lineArgs: [],
      stage: { verb: "Fetching", noun: "git log" },
    },
    {
      name: "git_add",
      variant: "GitAdd",
      aliases: ["git_stage", "stage", "add"],
      args: [{ name: "path", hint: '"all" or a file path', required: true }],
      summary: 'Stage a file\'s changes, or "all" to stage everything',
      group: "Git",
      approval: "always",
      autoInsert: false,
      timeoutMs: 10000,
      lineRe: /git_add\s*[(:]\s*["']([^"'\s)]+)["']?\s*\)?/i,
      lineArgs: ["path"],
      stage: { verb: "Staging", arg: "path", fallback: "changes" },
    },
    {
      name: "git_unstage",
      variant: "GitUnstage",
      aliases: ["unstage", "git_reset_file"],
      args: [{ name: "path", hint: "file path", required: true }],
      summary: "Unstage a file (working tree untouched)",
      group: "Git",
      approval: "always",
      autoInsert: false,
      timeoutMs: 10000,
      lineRe: /git_unstage\s*[(:]\s*["']([^"'\s)]+)["']?\s*\)?/i,
      lineArgs: ["path"],
      stage: { verb: "Unstaging", arg: "path", fallback: "file" },
    },
    {
      name: "git_commit",
      variant: "GitCommit",
      aliases: ["commit"],
      args: [{ name: "message", hint: "commit message", required: true }],
      summary: "Commit the currently staged changes",
      group: "Git",
      approval: "always",
      autoInsert: false,
      timeoutMs: 10000,
      lineRe: /git_commit\s*[(:]\s*["']([^"']+)["']\s*\)?/i,
      lineArgs: ["message"],
      stage: { verb: "Committing", noun: "the staged changes" },
    },
    {
      name: "git_branches",
      variant: "GitBranches",
      ...
      autoInsert: true,
      timeoutMs: 10000,
      lineRe: /git_branches\s*\(\s*\)/i,
      lineArgs: [],
      stage: { verb: "Fetching", noun: "git branches" },
    },
    {
      name: "git_create_branch",
      variant: "GitCreateBranch",
      aliases: ["create_branch", "git_branch_new"],
      args: [{ name: "name", hint: "branch name", required: true }],
      summary: "Create a new branch at the current HEAD",
      group: "Git",
      approval: "always",
      autoInsert: false,
      timeoutMs: 10000,
      lineRe: /git_create_branch\s*[(:]\s*["']([^"']+)["']?\s*\)?/i,
      lineArgs: ["name"],
      stage: { verb: "Creating", arg: "name", fallback: "branch" },
    },
    {
      name: "git_checkout",
      variant: "GitCheckout",
      aliases: ["checkout", "git_switch", "switch_branch"],
      args: [{ name: "branch", hint: "branch name", required: true }],
      summary: "Switch to a branch (refuses on a dirty tree)",
      group: "Git",
      approval: "destructive",
      autoInsert: false,
      timeoutMs: 10000,
      lineRe: /git_checkout\s*[(:]\s*["']([^"'\s)]+)["']?\s*\)?/i,
      lineArgs: ["branch"],
      stage: { verb: "Switching to", arg: "branch", fallback: "branch" },
    },
    {
      name: "git_commit_diff",
      variant: "GitCommitDiff",
      aliases: ["show_diff", "commit_diff"],
      args: [{ name: "commit", hint: "commit hash", required: true }],
      summary: "Show the patch of a specific commit",
      group: "Git",
      approval: "auto",
      autoInsert: true,
      timeoutMs: 10000,
      lineRe: /git_commit_diff\s*[(:]\s*["']([^"'\s)]+)["']?\s*\)?/i,
      lineArgs: ["commit"],
      stage: { verb: "Fetching", arg: "commit", fallback: "commit diff" },
    },
    {
      name: "git_show",
      variant: "GitShow",
      ...
      autoInsert: true,
      timeoutMs: 10000,
      lineRe: /git_show\s*[(:]\s*["']([^"'\s)]+)["']?\s*\)?/i,
      lineArgs: ["commit"],
      stage: { verb: "Showing", arg: "commit", fallback: "commit" },
    },
    {
      name: "describe_tool",
  ...
  function coerceArg(arg, raw) {
    ...
    // A collection arg (`paths[]`, `edits[]`) accepts a single scalar the
    // way the Rust parser's `string_array_arg` does — `{"paths":"a.ts"}` is
    // really `["a.ts"]`, not a malformed value.
    if (arg && arg.hint && arg.hint.includes("[]") && typeof raw === "string") {
      return [raw];
    }
